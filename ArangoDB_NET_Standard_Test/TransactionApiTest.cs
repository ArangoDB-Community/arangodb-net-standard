...
namespace ArangoDB_NET_Standard_Test
{
    public class TransactionApiTest: ApiTestBase
    {
        ...
        public async Task PostTransaction_ShouldSucceed()
        {
            ...
            var result = await _adb.Transaction.PostTransaction<List<PostDocumentResponse>>(new PostTransactionRequest
            {
                Action = @"
                    function (params) { 
        ...
        public async Task PostTransaction_ShouldThrow_WhenFunctionDefinitionHasSyntaxErrors()
        {
            var ex = await Assert.ThrowsAsync<ApiErrorException>(async () => 
                await _adb.Transaction.PostTransaction<object>(new PostTransactionRequest
                {
